...
const BlockedFriends = () => {
 ...
 useEffect(()=>{
    const dataErr = {
       errMsg: "Can,t access Blocked page without login/signUp",
       path1: "/signup",
       path2: "/signin",
    }
    if(!userDetails){
  ...
  useEffect(()=>{
    let path
    if(userDetails)
    path = `http://localhost:4000/friends/blockedfriends/${userDetails._id}`;
      ...
      if (!confirmed) return;
        ...
        try {
            const res = await axios.put(
              `http://localhost:4000/friends/unblock/${userDetails._id}/${id}`,
              {},
              {
          ...
          if(res.data.status === "success"){
            setHasFriends(false);
            const path = `http://localhost:4000/friends/blockedfriends/${userDetails._id}`;
            if (userDetails?._id) seeFriends(path, setData, setDetails, setHasFriends);
  ...
  return (
    ...
    {hasFriends && <Grid container spacing={8.5}>
      ...
      <Button
       ...
       onClick={() => handleUnblock(details.id)}
      >
        Unblock
      </Button>
      </CardActions>
