...
const RequestSent = () => {
 ...
 useEffect(()=>{
    const dataErr = {
       errMsg: "Can,t access Requests Sent page without login/signUp",
       path1: "/signup",
       path2: "/signin",
    }
    if(!userDetails){
  ...
  useEffect(()=>{
    let path;
    if(userDetails)
    path = `http://localhost:4000/friends/wishlist/${userDetails._id}`;
    ...
    const handleUnsend = async(id) => {
        try {
            const res = await axios.put(
              `http://localhost:4000/friends/withdraw/${userDetails._id}/${id}`,
              {},
              {
          ...
          if(res.data.status === "success"){
            setHasFriends(false);
            const path = `http://localhost:4000/friends/wishlist/${userDetails._id}`;
            if (userDetails?._id) seeFriends(path, setData, setDetails, setHasFriends);
  ...
  return (
    ...
    {hasFriends && <Grid container spacing={8.5}>
      ...
      <Button
       ...
       onClick={() => handleUnsend(details.id)}
      >
        Unsend
      </Button> 
      </CardActions>
    ...
    {!hasFriends && 
    <Typography variant="h3" sx={{color: 'whitesmoke', ml: '8rem', mt: '6rem'}}>
        You don't sent anyone a request
    </Typography>}
  </Box>
  );
};
